## Petalinux 安装配置和使用以及通过 linux 对 FPGA 的寄存器或者 RAM 进行读写
...
petalinux 对操作系统版本和依赖版本要求很高，经过测试在 ubuntu18.04 上安装失败。
...
描述：PS 和 PL 各有一个 led，通过 petalinux 产生一个完整 linux 系统，FPGA 端烧写一个 GPIO 控制器，输出 1 位信号到 R19 引脚，FPGA 的 R19 引脚连接了 pl 侧的 led 灯：
